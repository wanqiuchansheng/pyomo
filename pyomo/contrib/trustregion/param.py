...
from pyomo.util.config import PositiveInt, PositiveFloat, NonNegativeFloat, In
...
if CONFIG.get('sample region').value():
    default_sample_radius = 0.1
else:
    default_sample_radius = CONFIG.get('trust radius').value()/2.0
...
CONFIG.declare('radius max', ConfigValue(
        default = 1000.0 * CONFIG.get('trust radius').value(),
        domain = PositiveFloat,
        description = '',
        doc = ''))
